...
/// اعمال دی‌ان‌اس در لینوکس با اولویت NetworkManager (nmcli) و resolvectl
pub fn set_system_dns(primary: String, secondary: String) -> Result<String, String> {
    ...
    // ۱. استفاده از NetworkManager (nmcli) که سیستم استاندارد کالی لینوکس است
    let nm_out = Command::new("nmcli")
        .args(&["-t", "-f", "NAME,DEVICE", "connection", "show", "--active"])
        .output();

    if let Ok(out) = nm_out {
        if let Ok(text) = String::from_utf8(out.stdout) {
            for line in text.lines() {
                let conn_name = line.split(':').next().unwrap_or("").trim();
                if !conn_name.is_empty() && !conn_name.contains("tun") {
                    let _ = Command::new("nmcli")
                        .args(&["connection", "modify", conn_name, "ipv4.dns", &format!("{} {}", primary_ip, secondary_ip), "ipv4.ignore-auto-dns", "yes"])
                        .output();
                    let _ = Command::new("nmcli").args(&["connection", "up", conn_name]).output();
                    *process_guard = Some((primary, secondary));
                    return Ok("دی‌ان‌اس با موفقیت از طریق NetworkManager فعال شد.".to_string());
                }
            }
        }
    }

    // ۲. روش جایگزین resolvectl
    ...
    // ۳. استفاده از اسکریپت کمکی sudoers اگر وجود داشته باشد
    let helper_cmd = Command::new("sudo")
        .args(&["-n", "/opt/redcloud/dns-helper", "set", &primary_ip.to_string(), &secondary_ip.to_string()])
        .output();

    if let Ok(out) = helper_cmd {
        if out.status.success() {
            *process_guard = Some((primary, secondary));
    ...
    Err("خطا در اعمال دی‌ان‌اس. لطفاً بررسی کنید NetworkManager یا systemd-resolved فعال باشد.".to_string())
}

/// ریست کردن تنظیمات DNS لینوکس به وضعیت پیش‌فرض (DHCP)
pub fn reset_system_dns() -> Result<String, String> {
    let mut process_guard = ACTIVE_DNS.lock().unwrap_or_else(|e| e.into_inner());

    // ریست در NetworkManager
    let nm_out = Command::new("nmcli")
        .args(&["-t", "-f", "NAME,DEVICE", "connection", "show", "--active"])
        .output();
    ...
    if let Ok(out) = nm_out {
        if let Ok(text) = String::from_utf8(out.stdout) {
            for line in text.lines() {
                ...
                if !conn_name.is_empty() && !conn_name.contains("tun") {
                    let _ = Command::new("nmcli")
                        .args(&["connection", "modify", conn_name, "ipv4.dns", "", "ipv4.ignore-auto-dns", "no"])
                        .output();
                    let _ = Command::new("nmcli").args(&["connection", "up", conn_name]).output();
                }
            }
    ...
    let _ = Command::new("sudo").args(&["-n", "/opt/redcloud/dns-helper", "reset"]).output();

    *process_guard = None;
    Ok("تنظیمات دی‌ان‌اس سیستم لینوکس به حالت خودکار (DHCP) بازگشت.".to_string())
}
...
fn load_deep_scan_ips() -> Vec<String> {
    ...
    if candidate_ips.is_empty() {
        let fallback_cidrs = vec![
            ...
            "104.25.0.0/24", "104.26.0.0/24", "104.27.0.0/24",
