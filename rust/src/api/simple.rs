...
// ذخیره درصد پیشرفت پیش‌فرض لود شدن تور در حافظه راست
...
pub fn start_proxy_with_node(
    ...
    // تولید پویای بخش DNS بر اساس مقادیر جدید ورودی از فلاتر
    let dns_server_json = match dns_type.as_str() {
        "doh" => {
            let url = dns_doh_url.clone().unwrap_or_else(|| "https://cloudflare-dns.com/dns-query".to_string());
        ...
        "dot" => {
            let server_name = dns_dot_host.clone().unwrap_or_else(|| "dns.google".to_string());
            serde_json::json!({
                "type": "tls",
                "tag": "dns_proxy",
                "server": dns_primary,
