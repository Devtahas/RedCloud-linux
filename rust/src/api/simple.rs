...
fn resolve_binary_path(name: &str) -> PathBuf {
    ...
    if let Ok(cur) = std::env::current_dir() {
        let candidate = cur.join(clean_name);
        if candidate.exists() {
            ensure_executable(&candidate);
            return candidate;
        }
...
/// پیکربندی پیشرفته پروسه در لینوکس: اتصال طول عمر پروسه فرزند به والد
#[cfg(target_os = "linux")]
fn configure_linux_child_lifecycle(command: &mut Command) {
    unsafe {
        command.pre_exec(|| {
...
/// کشتن قطعی پردازه‌های باقیمانده بر اساس نام در لینوکس
...
/// اعمال دی‌ان‌اس در لینوکس (پشتیبانی از resolvectl و /etc/resolv.conf)
pub fn set_system_dns(primary: String, secondary: String) -> Result<String, String> {
    ...
    let work_dir = get_safe_work_dir();
    let resolv_backup = work_dir.join("resolv.conf.backup");

    // بک‌آپ از resolv.conf اصلی اگر قبلاً بک‌آپ گرفته نشده باشد
    if !resolv_backup.exists() {
        if let Ok(original_resolv) = fs::read_to_string("/etc/resolv.conf") {
            let _ = fs::write(&resolv_backup, original_resolv);
        }
    }

    // ۱. ابتدا تلاش برای اعمال از طریق systemd-resolved (resolvectl)
    let resolvectl_cmd = Command::new("resolvectl")
        .args(&["dns", "@default", &primary_ip.to_string(), &secondary_ip.to_string()])
        .output();

    if let Ok(out) = resolvectl_cmd {
        if out.status.success() {
            let _ = Command::new("resolvectl").args(&["domain", "@default", "~."]).output();
            *process_guard = Some((primary, secondary));
            return Ok("دی‌ان‌اس با موفقیت از طریق resolvectl در لینوکس فعال شد.".to_string());
        }
    }

    // ۲. روش جایگزین مستقیم با ویرایش /etc/resolv.conf
    let new_resolv_content = format!(
        "# Generated by RedCloud VPN\nnameserver {}\nnameserver {}\n",
        primary_ip, secondary_ip
    );

    let temp_resolv = work_dir.join("temp_resolv.conf");
    if fs::write(&temp_resolv, &new_resolv_content).is_ok() {
        // تلاش برای کپی امن با دسترسی
        let copy_res = Command::new("cp")
            .args(&[temp_resolv.to_str().unwrap(), "/etc/resolv.conf"])
            .output();

        if let Ok(out) = copy_res {
            if out.status.success() {
                *process_guard = Some((primary, secondary));
                return Ok("دی‌ان‌اس با موفقیت در /etc/resolv.conf اعمال شد.".to_string());
            }
        }
    }

    Err("خطا در اعمال دی‌ان‌اس. لطفاً برنامه را با sudo اجرا کنید یا دسترسی دستکاری فایل‌های شبکه را تنظیم کنید.".to_string())
}

/// ریست کردن تنظیمات DNS لینوکس به وضعیت پیش‌فرض
pub fn reset_system_dns() -> Result<String, String> {
    let mut process_guard = ACTIVE_DNS.lock().unwrap_or_else(|e| e.into_inner());

    // ریست از طریق resolvectl
    let _ = Command::new("resolvectl").args(&["revert", "@default"]).output();

    let work_dir = get_safe_work_dir();
    let resolv_backup = work_dir.join("resolv.conf.backup");

    if resolv_backup.exists() {
        if let Ok(content) = fs::read_to_string(&resolv_backup) {
            let temp_restore = work_dir.join("temp_restore.conf");
            let _ = fs::write(&temp_restore, content);
            let _ = Command::new("cp").args(&[temp_restore.to_str().unwrap(), "/etc/resolv.conf"]).output();
            let _ = fs::remove_file(&resolv_backup);
        }
    }

    *process_guard = None;
    Ok("تنظیمات دی‌ان‌اس سیستم لینوکس به حالت پیش‌فرض بازگشت.".to_string())
}

/// تنظیم پروکسی سراسری لینوکس (سازگار با GNOME، XFCE، Cinnamon، MATE و KDE)
fn set_linux_system_proxy(enable: bool, host: String, port: u16, is_socks: bool) {
    if enable {
        // ۱. تنظیم برای محیط‌های دسکتاپ بر پایه GNOME (GNOME, Kali Desktop, Cinnamon, Ubuntu, XFCE)
        ...
        // ۲. تنظیم برای محیط KDE Plasma
        let _ = Command::new("kwriteconfig5")
            .args(&["--file", "kioslaurc", "--group", "Proxy Settings", "--key", "ProxyType", "1"])
            .output();
        let _ = Command::new("kwriteconfig6")
            .args(&["--file", "kioslaurc", "--group", "Proxy Settings", "--key", "ProxyType", "1"])
            .output();
    } else {
        // غیرفعال‌سازی در GNOME
        let _ = Command::new("gsettings")
            .args(&["set", "org.gnome.system.proxy", "mode", "none"])
            .output();

        // غیرفعال‌سازی در KDE Plasma
        let _ = Command::new("kwriteconfig5")
            .args(&["--file", "kioslaurc", "--group", "Proxy Settings", "--key", "ProxyType", "0"])
            .output();
        let _ = Command::new("kwriteconfig6")
            .args(&["--file", "kioslaurc", "--group", "Proxy Settings", "--key", "ProxyType", "0"])
            .output();
...
fn spawn_single_aether_mode(
    ...
    let mut child = command.spawn()
        .map_err(|e| format!("خطا در اجرای aether در مسیر {:?}: {}", binary_path, e))?;
...
pub fn start_aether_core(
    ...
    if let Some(c) = connected_child {
        ...
        *process_guard = Some(c);
        if use_system_proxy {
            set_linux_system_proxy(true, "127.0.0.1".to_string(), 1820, false);
        }
...
pub fn stop_aether_core() -> Result<String, String> {
    ...
    set_linux_system_proxy(false, String::new(), 0, false);
...
pub fn start_hybrid_connection(
    ...
    if use_system_proxy && !use_tun_mode {
        set_linux_system_proxy(true, "127.0.0.1".to_string(), 2080, false);
    }

    Ok("اتصال ترکیبی هیبریدی در لینوکس با موفقیت برقرار شد!".to_string())
}
...
fn start_tor_core_internal(
    ...
    let mut child = command.spawn()
        .map_err(|e| format!("خطا در اجرای فرآیند تور در مسیر {:?}: {}", resolved_path, e))?;
    ...
    *process_guard = Some(child);
    ...
    if use_system_proxy {
        set_linux_system_proxy(true, "127.0.0.1".to_string(), 9051, false);
    }
...
fn start_psiphon_core_internal(
    ...
    if use_system_proxy {
        set_linux_system_proxy(true, "127.0.0.1".to_string(), 9081, false);
    }
    
    Ok("در حال برقراری اتصال با سرورهای سایفون...".to_string())
}
    ...
    match child {
        Ok(c) => {
            ...
            *process_guard = Some(c);
            ...
            Ok("اتصال با موفقیت در لینوکس برقرار شد.".to_string())
        }
        Err(e) => Err(format!("خطا در اجرای فرآیند هسته در مسیر {:?}: {}", resolved_path, e)),
    }
}
